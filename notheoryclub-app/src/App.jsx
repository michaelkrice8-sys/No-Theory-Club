...
function SongBuilder({ audio, chordVariants, updateVariant }) {
  ...
  const copySection = (id) => {
    setSections(s=>{
      ...
      const copy={ ...orig, id:Date.now()+Math.random(), name:orig.name+" (copy)",
        rows:orig.rows.map(r=>({...r, id:Date.now()+Math.random(),
          strumActive:[...r.strumActive], blockChords:[...r.blockChords]})) };
  ...
  // ── Row operations ──
  const updateRow = (secId,rowIdx,fn) => setSections(s=>s.map(sec=>sec.id!==secId?sec:{
    ...sec, rows:sec.rows.map((r,i)=>i!==rowIdx?r:fn(r)) }));
  ...
  const handleBlockClick = (secId,rowIdx,colIdx,inAssign) => {
    if(inAssign){
      updateRow(secId,rowIdx,r=>{
        const bc=[...r.blockChords];
        bc[colIdx]=bc[colIdx]===assignChord?null:assignChord;
        return {...r,blockChords:bc};
      });
    } else {
      updateRow(secId,rowIdx,r=>{
        const sa=[...r.strumActive]; sa[colIdx]=!sa[colIdx]; return {...r,strumActive:sa};
      });
    }
  };

  const capoBtnStyle = { width:22, height:22, borderRadius:6, border:"1px solid #333",
    background:"#1a1a1a", color:"#aaa", fontSize:15, cursor:"pointer",
    display:"flex", alignItems:"center", justifyContent:"center" };
      ...
      <div style={{ width:"100%", background:"#0a0a0a", border:"1px solid #2a2a2a",
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div style={{ display:"flex", alignItems:"center", gap:8,
            background:"#111", border:"1px solid #2a2a2a", borderRadius:10, padding:"6px 12px" }}>
          ...
          <button onClick={()=>setMuteClick(m=>!m)} style={{
            ...
            {muteClick?"🔇 Muted":"🔔 Click"}
          </button>
          <div style={{ padding:"8px 16px", borderRadius:10, border:"1px solid #2a2a2a",
            background:"#111", color:"#444", fontSize:12, fontWeight:700 }}>
      ...
      {sections.map((sec,idx)=>{
        ...
        return (
          <div key={sec.id}
            ...
            style={{
              width:"100%", background:"#0a0a0a", border:`1px solid ${isDragOver?"#FFBE0B":"#2a2a2a"}`,
              borderRadius:18, marginBottom:14,
              opacity:isDragging?0.45:1,
              transition:"opacity 0.15s, border-color 0.15s",
              boxShadow:isDragOver?"0 0 12px rgba(255,190,11,0.3)":"none",
            }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:8, padding:"12px 14px",
              ...
              {/* Drag handle */}
              <div style={{ fontSize:18, color:"#444", cursor:"grab", padding:"0 4px",
                userSelect:"none", flexShrink:0 }}>☰</div>
              ...
              {editingId===sec.id
                ? <input autoFocus value={editingName}
                    ...
                    style={{ flex:1, background:"transparent", border:"none", borderBottom:"1px solid #FFBE0B",
                      color:"#fff", fontSize:14, fontWeight:800, outline:"none", padding:"2px 0" }} />
                : <div onClick={()=>{ setEditingId(sec.id); setEditingName(sec.name); }}
                    style={{ flex:1, fontSize:14, fontWeight:800, color:"#fff", cursor:"text" }}>
                    {sec.name}
                  </div>
              }

              {/* Actions */}
              <button onClick={()=>copySection(sec.id)} title="Duplicate" style={{
                background:"none", border:"none", color:"#555", fontSize:15, cursor:"pointer",
                padding:"4px 6px", borderRadius:6 }}>⧉</button>
              <button onClick={()=>{ setAssignSectionId(isAssigning?null:sec.id); }} style={{
                padding:"4px 10px", borderRadius:8,
                border:isAssigning?"2px solid #FFBE0B":"1px solid #2a2a2a",
                background:isAssigning?"rgba(255,190,11,0.12)":"transparent",
                color:isAssigning?"#FFBE0B":"#555", fontSize:11, fontWeight:700, cursor:"pointer" }}>
                {isAssigning?"🎸":"✏️"}
              </button>
              <button onClick={()=>deleteSection(sec.id)} title="Delete" style={{
                background:"none", border:"none", color:"#e74c3c44", fontSize:15, cursor:"pointer",
                padding:"4px 6px", borderRadius:6 }}>✕</button>
            </div>
            ...
            <div style={{ padding:"12px 14px" }}>
              {sec.rows.map((row,rowIdx)=>(
                <div key={row.id} style={{ marginBottom:rowIdx<sec.rows.length-1?14:0 }}>
                  {/* Row controls */}
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:6, marginBottom:6 }}>
                    <span style={{ fontSize:9, color:"#444", letterSpacing:1 }}>ROW {rowIdx+1}</span>
                    <button onClick={()=>updateRow(sec.id,rowIdx,r=>({...r,size:cycleSize(r.size),
                      strumActive:defaultBuild(cycleSize(r.size)),blockChords:Array(cycleSize(r.size)).fill(null)}))} style={{
                    ...
                    <button onClick={()=>updateRow(sec.id,rowIdx,r=>({...r,repeat:r.repeat>=4?1:r.repeat+1}))} style={{
                      ...
                      color:row.repeat>1?"#FFBE0B":"#555",
                      fontSize:11, fontWeight:700, cursor:"pointer" }}>
                      {row.repeat}× 🔁
                    </button>
                    {sec.rows.length>1&&(
                      <button onClick={()=>removeRow(sec.id,rowIdx)} style={{
                        background:"none", border:"none", color:"#e74c3c44",
                        fontSize:13, cursor:"pointer", padding:"2px 4px" }}>✕</button>
                    )}
                  </div>
                  {/* Blocks */}
                      ...
                      return (
                        <div key={colIdx} style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:2 }}>
                          ...
                          <div style={{ fontSize:13, fontWeight:900, height:18, lineHeight:"18px",
                            ...
                            textShadow:ch?"0 0 6px rgba(255,190,11,0.5)":"none" }}>{ch||"·"}</div>
                        </div>
                      );
                    })}
                  </div>
              ...
              <div style={{ display:"flex", gap:8 }}>
                <input autoFocus value={saveName} onChange={e=>setSaveName(e.target.value)}
                  ...
                  style={{ flex:1, padding:"9px 12px", borderRadius:10,
                    border:"1px solid #333", background:"#0a0a0a",
                    color:"#fff", fontSize:13, outline:"none" }} />
                <button onClick={handleSave} style={{ padding:"9px 16px", borderRadius:10, border:"none",
                  background:"linear-gradient(135deg,#FFBE0B,#F77F00)",
                  color:"#111", fontSize:13, fontWeight:800, cursor:"pointer" }}>Save</button>
                <button onClick={()=>{setSavePrompt(false);setSaveName("");}} style={{
                  padding:"9px 12px", borderRadius:10, border:"1px solid #333",
        ...
        <div style={{ marginBottom:10, background:"#111", border:"1px solid #FFBE0B33", borderRadius:14, padding:"14px" }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            ...
            <button onClick={doSave} style={{ padding:"9px 16px", borderRadius:10, border:"none",
              background:"linear-gradient(135deg,#FFBE0B,#F77F00)",
              color:"#111", fontSize:13, fontWeight:800, cursor:"pointer" }}>Save</button>
            <button onClick={()=>setStrumSavePrompt(false)} style={{
              padding:"9px 12px", borderRadius:10, border:"1px solid #333",
              background:"transparent", color:"#555", fontSize:13, cursor:"pointer" }}>✕</button>
          </div>
        </div>
